...
app.post('/api/auth/login', (req, res) => {
  ...
  setAuthCookies(res, createAccessToken(user.id, emailLower), createRefreshToken(user.id));
  res.json({
...
app.post('/api/enquiry/submit', (req, res) => {
  ...
  res.json({ message: 'Enquiry submitted successfully', enquiry_id: enquiry.id });
});

// ===================
// ADMIN
// ===================
app.get('/api/admin/enquiries', getCurrentUser, requireRole('admin'), (req, res) => {
  res.json(enquiries);
});

app.post('/api/admin/approve-employer', getCurrentUser, requireRole('admin'), (req, res) => {
  ...
  res.json({ message: 'Employer approved and account created', employer_id: newEmployer.id });
});

app.get('/api/admin/users', getCurrentUser, requireRole('admin'), (req, res) => {
  res.json(users.map(publicUser));
});

app.get('/api/admin/advance-requests', getCurrentUser, requireRole('admin'), (req, res) => {
...
// EMPLOYER
// ===================
app.post('/api/employer/add-employee', getCurrentUser, requireRole('employer'), (req, res) => {
  ...
  res.json({ message: 'Employee added successfully', employee_id: newEmployee.id });
});

app.get('/api/employer/employees', getCurrentUser, requireRole('employer'), (req, res) => {
  const list = users
    .filter(u => u.role === 'employee' && u.company_id === req.currentUser.id)
    .map(publicUser);
  res.json(list);
});

app.get('/api/employer/advance-requests', getCurrentUser, requireRole('employer'), (req, res) => {
  const list = advanceRequests.filter(r => r.employer_id === req.currentUser.id);
  res.json(list);
});

app.post('/api/employer/handle-request', getCurrentUser, requireRole('employer'), (req, res) => {
  ...
  if (action === 'reject' && rejection_reason) r.rejection_reason = rejection_reason;
...
// EMPLOYEE
// ===================
app.post('/api/employee/request-advance', getCurrentUser, requireRole('employee'), (req, res) => {
  ...
  const pending = advanceRequests.find(r => r.employee_email === user.email && r.status === 'pending');
  if (pending) return res.status(400).json({ detail: 'You already have a pending request' });
  ...
  res.json({ message: 'Advance request submitted successfully', request_id: newRequest.request_id });
});

app.get('/api/employee/my-requests', getCurrentUser, requireRole('employee'), (req, res) => {
  const user = users.find(u => u.id === req.currentUser.id);
  const list = advanceRequests.filter(r => r.employee_email === user.email);
  res.json(list);
});

app.get('/api/employee/dashboard-stats', getCurrentUser, requireRole('employee'), (req, res) => {
  const user = users.find(u => u.id === req.currentUser.id);
  if (!user) return res.status(404).json({ detail: 'User not found' });
  ...
  const advanceLimitPct = user.advance_limit_percentage || 30;
  const maxAdvance = (monthlySalary * advanceLimitPct) / 100;
  const approved = advanceRequests.filter(r => r.employee_email === user.email && r.status === 'approved');
